...
from .old import exec_config 


# After import *, old settings are now in globals
old_settings = globals()


# Build paths inside the project like this: BASE_DIR / 'subdir'.
BASE_DIR = old_settings.get("BASE_DIR", Path(__file__).resolve().parent.parent)
...
old_apps = old_settings.get("INSTALLED_APPS", []) #!: duck-modified

if old_apps:
    INSTALLED_APPS = old_apps
...
old_middlewares = old_settings.get('MIDDLEWARES', [])

if old_middlewares:
    MIDDLEWARES = old_middlewares
...
ROOT_URLCONF = f"{exec_config.main_app_name}.urls"
...
WSGI_APPLICATION = f"{exec_config.main_app_name}.wsgi.application"
...
DATABASES.update(old_settings.get("DATABASES", {}))
...
auth_validators = old_settings.get("AUTH_PASSWORD_VALIDATORS", [])
...
LANGUAGE_CODE = old_settings.get('LANGUAGE_CODE', "en-us")


TIME_ZONE = old_settings.get('TIME_ZONE', "UTC")


USE_I18N = old_settings.get('USE_I18N', True)


USE_TZ = old_settings.get('USE_TZ', True)
...
DEFAULT_AUTO_FIELD = old_settings.get('DEFAULT_AUTO_FIELD', "django.db.models.BigAutoField")
